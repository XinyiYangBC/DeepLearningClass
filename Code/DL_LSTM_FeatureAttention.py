...
# =====================================================
# 5. Model Definition (LSTM + Feature Gating)
# =====================================================
class LSTMFire_FeatAttn(nn.Module):
    def __init__(self, input_dim, hidden_dim=64, num_layers=1, dropout=0.2):
        ...
        last_gate_layer = self.feat_gate[-1]
        if isinstance(last_gate_layer, nn.Linear):
            nn.init.zeros_(last_gate_layer.bias)
    ...
    def forward(self, x):
        ...
        gate_scores = self.feat_gate(x)         
        alpha_feat = torch.sigmoid(gate_scores)  
        x_weighted = alpha_feat * x             
        h_seq, _ = self.lstm(x_weighted)       
        h_last = h_seq[:, -1, :]              
        out = self.fc(h_last)                

        return out, alpha_feat


# =====================================================
# 6️. Training (with Best Model Saving)
# =====================================================
model = LSTMFire_FeatAttn(input_dim=X.shape[1], hidden_dim=hidden_dim).to(device)
...
# =====================================================
model.eval()
preds, trues, attn_feat_all = [], [], []
with torch.no_grad():
    for xb, yb in test_loader:
        xb = xb.to(device)
        pred, alpha_feat = model(xb)
...
attn_feat = torch.cat(attn_feat_all, dim=0).mean(dim=(0,1)).numpy()  # [F]
...
# =====================================================
# 10. Visualize Feature-wise Attention (Gates)
# =====================================================
